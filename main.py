...
def train_epoch(models, criterion, optimizers, dataloaders,
    ...
    for data in tqdm(dataloaders['train'], leave=False,
        ...
        # Visualize
        if (iters % 100 == 0) and (vis is not None) \
            ...
            vis.line(
                X=np.stack(
                    [np.array(plot_data['X'])] * len(plot_data['legend']), 1),
...
if __name__ == '__main__':
    ...
    for trial in range(TRIALS):
        ...
        for cycle in range(CYCLES):
            ...
            # Training and test
            train(models, criterion, optimizers, schedulers,
                  dataloaders, EPOCH, EPOCHL, vis, plot_data)
            acc = test(models, dataloaders, mode='test')
            print('Trial {}/{} || Cycle {}/{} || Label set size {}: \
                  Test acc {}'.format(trial+1, TRIALS, cycle+1,
